...
// Memoize the tech color mapping to prevent recalculation
const techColorCache = new Map<string, TechColor>();
...
const getTechColor = (tech: string): TechColor => {
  ...
  const colors: Record<string, TechColor> = {
    ...
    next: {
      bg: "bg-black/10 dark:bg-white/20",
    ...
    tensorflow: {
      bg: "bg-yellow-500/10 dark:bg-amber-500/20",
      text: "text-amber-600 dark:text-amber-400",
      border: "border-amber-500/20",
    },
    pytorch: {
...
const safeCalculateDuration = (
  start: string | Date,
  end: string | Date | "Present"
): string => {
  const startDate = start instanceof Date ? start : new Date(start);
  return calculateDuration(
    startDate,
    end === "Present" ? "Present" : end instanceof Date ? end : new Date(end)
  );
};
...
const ProjectCard = React.memo(({
  ...
  const hasVideoPreviews =
    project.videoPreviews && project.videoPreviews.length > 0;
      ...
      <div className="h-full">
        ...
        >
            ...
            {!isImageLoaded ? (
              ...
              <div className="relative w-full h-full">
                ...
                {!isVideoPlaying && isVideoLoaded && (
                  ...
                  >
                    <div className="bg-black/50 p-3 rounded-full"></div>
                  </div>
                )}
              </div>
            ) : (
          ...
          <m.div
            ...
            className="p-6 space-y-4 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors duration-300"
            onClick={() =>
              (window.location.href = `/projects/${project.title.toLowerCase().replace(/\s+/g, "-")}`)
            }
          ...
          >
            <div className="p-5 flex flex-col relative z-10">
              ...
              <div className="flex items-center text-sm text-zinc-400 space-x-4">
                ...
                <span>
                  {safeCalculateDuration(project.startDate, project.endDate)}
                </span>
              </div>
            </div>
            ...
            <div
              className="flex gap-4 pt-2 relative z-10"
              onClick={(e) => e.stopPropagation()}
            >
              {project.github && (
...
const Projects = ({ showAll = false, maxItems = 3 }: ProjectsProps) => {
  ...
  const displayedProjects = useMemo(() => 
    showAll ? projects : projects.slice(0, maxItems)
  , [showAll, maxItems]);

  // Only run animations after component mounts to prevent hydration issues
  React.useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  if (!isMounted) {
    return (
      <div 
        id="projects"
        className="w-full max-w-7xl mx-auto px-4 sm:px-6 pt-20 -mt-20"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayedProjects.map((_, index) => (
            <div key={`skeleton-${index}`} className="h-64 bg-zinc-200 dark:bg-zinc-800 rounded-xl animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div
      id="projects"
      className="w-full max-w-7xl mx-auto px-4 sm:px-6 pt-20 -mt-20 transition-colors duration-300"
    >
      {!showAll && (
        <m.div

...
const Comments = () => {
  ...
  const updateUserComments = async (
    userId: string,
    updates: { displayName?: string; photoURL?: string | null | undefined }
  ) => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user.displayName) {
      setError(
        "Please set up your profile with a display name before commenting"
      );
      return;
    }
      ...
      if ((type === "up" && hasUpvoted) || (type === "down" && hasDownvoted)) {
        ...
        await updateDoc(commentRef, {
          [type === "up" ? "upvotes" : "downvotes"]:
            (comment[type === "up" ? "upvotes" : "downvotes"] || 0) - 1,
          [type === "up" ? "upvoters" : "downvoters"]: arrayRemove(user.uid),
        });
      } else if (
        (type === "up" && hasDownvoted) ||
        (type === "down" && hasUpvoted)
      ) {
        // Switch vote
        await updateDoc(commentRef, {
          [type === "up" ? "upvotes" : "downvotes"]:
            (comment[type === "up" ? "upvotes" : "downvotes"] || 0) + 1,
          [type === "up" ? "downvotes" : "upvotes"]:
            (comment[type === "up" ? "downvotes" : "upvotes"] || 0) - 1,
        ...
        <div className="space-y-6">
          {isLoading ? (
            ...
            comments.map((comment) => {
              ...
              const canModify =
                user &&
                (user.email === ADMIN_EMAIL || user.uid === comment.uid);
                        ...
                        {comment.upvoters?.some((uid) => {
                          ...
                          <div className="absolute -top-4 -right-4 flex items-center justify-center">
                            <div className="w-6 h-6 rounded-full overflow-hidden border-2 border-yellow-500 bg-white">
                              <Image
                                src={
                                  comments.find((c) => c.email === ADMIN_EMAIL)
                            ...
                            <Heart
                              className="w-3 h-3 text-red-500 absolute -bottom-1 -right-1"
                              fill="currentColor"
                            />
                          </div>
                        )}
                      ...
                      >
                        ...
                        {comment.downvoters?.some((uid) => {
                          ...
                          <div className="absolute -top-4 -right-4 flex items-center justify-center">
                            ...
                            <Heart
                              className="w-3 h-3 text-amber-500 absolute -bottom-1 -right-1"
                              fill="currentColor"
                            />
                          </div>
                        )}
